...
const getTimePresent = () => {
    ...
    var datePresentReverse = (today.getMonth() + 1) + '/' + today.getDate() + '/' + today.getFullYear()
...
modalEditFood.addEventListener('show.bs.modal', (event) => {
    ...
    const { datePresentReverse } = getTimePresent()
    ...
    btnCompleteEdit.onclick = (e) => {
        ...
        if (completeTimeEdit === time) {
            toastFail('Sửa đổi thất bại. Bạn không có sự thay đổi nào');
        ...
        settingFoods.forEach((value) => {
            if (value.time != time) {
                ...
                var date1 = new Date(`${datePresentReverse} ${completeTimeEdit}`);
        ...
        if (checkData.length && !checkData.includes(1)) {
            ...
            toastFail('Sửa thất bại. Các mốc thời gian phải cách nhau 30 phút');
            $('#modalEditFood').modal('hide');
            $('#loading_edit_food').hide()
            btnCompleteEdit.classList.remove('d-none')
                ...
                const tableContent = foodData.map((v, index) => {
                    ...
                    return `
                                    <tr class="text-center">
                                        ...
                                        <td>${v.amount_food}</td>
                                        <td > <span style="border-radius : 5px ; background-color : ${style}; ">${v.status
                        }</span></td>
                                    <td> <button class="btn_action_food" data-id-food="${id}" data-time-food="${v.time
                        }" data-amount-food="${v.amount_food
...
btnCompleteSetting.onclick = (e) => {
    ...
    const settingFoods = JSON.parse(localStorage.getItem('setting_food'))
    const userNameLogin = document.querySelector('#username_login').innerHTML;
    const { datePresentReverse, timePresent } = getTimePresent()
    ...
    if (checkData.length && !checkData.includes(1)) {
        ...
        if (settingFoods.length > 10) {
            toastFail('Tối đa chỉ được 10 cài đặt');
            return
        }
        ...
        return toastFail('Các môc thời gian phải cách nhau 30 phút');
    }
